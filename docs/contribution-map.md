...
이 문서는 이시원(`NearthYou`)이 작성자로 표시된 원본 commit을 source file과 현재 위치에 연결합니다. standalone 저장소는 원본 Git history를 복제하지 않았으므로 commit 링크와 원본 저장소가 기여 확인의 Source of Truth입니다.
...
- 원본에서 선별하지 않은 실험 스크립트·보고서는 `현재 위치`에 “미이관”으로 표시합니다.

## GPT lab personal commits
...
| [`cb69bdf`](https://github.com/Soldbone/gpt-lab/commit/cb69bdf) | experiment/reproducibility scripts and tests | 미이관; `docs/results.md`에서 provenance만 참조 | experiment output·reproducibility test 보강 | 이시원 (`NearthYou`) |
...
## NumPy/MNIST lab personal commits
...
기여 map은 “누가 어떤 commit을 작성했는가”를 증명하지만, 해당 file의 모든 줄이 한 사람만의 작업이라는 뜻은 아닙니다. 이후 team merge와 수정은 원본 history를 함께 확인해야 합니다.
