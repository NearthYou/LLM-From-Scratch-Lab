...
이 저장소는 신경망의 기본 연산을 직접 구현하는 단계와, PyTorch로 autoregressive language model을 조립하는 단계를 분리합니다.
...
이 단계의 목적은 모델 정확도 한 줄보다 tensor shape, cached state, gradient 흐름, train/eval mode 차이를 코드와 unit test로 확인하는 것입니다.
...
## 3. Embedding and causal attention
...
## 4. GPT block and language-model head
...
## 5. Training, checkpoint, generation, fine-tuning
...
- `foundations/mnist_numpy/tests/`: NumPy layer·optimizer·network 동작
...
Historical GPU experiment와 current CPU smoke run은 설정·데이터·목적이 다르므로 직접 비교하지 않습니다.
